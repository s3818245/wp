...
<body data-spy="scroll" data-target=".navbar" data-offset="50">
    ...
    if(!empty($_POST['edit-acc'])){
        $_SESSION['edit-id'] = $_POST['edit-acc'];
    }

    if(!empty($_SESSION['edit-id'])){
        $errorFound = 0;

        $currentUser = $_SESSION['edit-id'];

        $sql = "SELECT * FROM userData WHERE username='$currentUser';";
    ...
    if(isset($_POST['update-acc'])){
            ...
            if($errorFound==0){
                $sql = "UPDATE userData SET username='$username', `password`='$userpassword' WHERE username='$currentUser';";
            }
            ...
            <div class="col-6 my-auto" style="border: 1px black solid;">
                ...
                <form action="<?php htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST" name="add-account">
                    <div class="form-group">
                        ...
                        <input type="text" class="Input form-control" name="add-username" id="add-user" placeholder="<?php echo $currentusername;?>">
                        <span style="color: red;"><?php echo $usernameErr?></span>
                    </div>
                    <br>
                    <div class="form-group">
                        <label for="password">Password:</label>
                        <input type="password" class="Input form-control" name="add-password" id="add-pass" placeholder="<?php echo $currentpassword;?>">
                        <span style="color: red;"><?php echo $passwordErr?></span>
                    </div>
                    <br>
                    <div class="float-right">
                        <button class="" name="update-acc"type="submit" value="Submit">Update account</button>
                    </div>
                </form>
            </div>
        </div>
